...
class PongService:
    ...
    def send_message(self, message_id, size):
        msg = b'\0' * size
        future = self.producer.send("ping", value=msg)
        future.add_errback(self._on_send_error)

    @log_stats
    def handle_message(self, message):
        self.send_message(message_id=None, size=message.serialized_value_size)

    def compute_stats(self, total_time, num_messages, size):
        stats = {
    ...
    def send(self, num_messages, size=10):
        ...
        stats = self.compute_stats(end-start, num_messages, size)
